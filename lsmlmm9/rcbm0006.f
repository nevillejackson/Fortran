...
C     ----------------------------------------------------              DK050060          
     ...
     2DFA,IFPC,NDFC,NDFAC,NINT                                          DK050310          
      DATA IBLK,JBLK/55555555555555555555B,00000000000000000055B/                         
      DATA IZER/00000000000000000033B/                                                    
      REWIND 2                                                          DK050330          
 ...
 1001 FORMAT (1H1,20X,35HLISTING OF X MATRIX FOR PROBLEM NO.,I3)        DK050370          
    ...
    5 RHM(I)=0.0                                                        DK050430          
      MIN=0                                                             DK050440          
      IF(NCPR.EQ.0) GO TO 6                                             DK050450          
      KA=NRHM*(NRHM+1)/2                                                DK050460          
   ...
   11 DO 8 I=1,KA                                                       DK050550          
    ...
    9 TOT3(I)=0.0                                                       DK050610          
      ...
      GO TO 228                                                         DK050740          
  227 DO 229 I=1,99                                                     DK353750          
  229 TOT4(I)=R2I(I)                                                    DK050760          
  228 IF (KB.EQ.1.AND.MTY.GT.5) GO TO 225                               DK050770          
      GO TO 9999                                                                          
  225 DO 226 I=1,100                                                    DK050790          
      R1I(I)=0.0                                                        DK050800          
  226 R2I(I)=0.0                                                        DK050810          
C-----INITIALIZE READ ROUTINE                                                             
 9999 CALL ROPEN(IN,K,L,IC,1,NTRA)                                                        
C-----START CARD READING LOOP                                                             
   20 K=1                                                               DK050820          
      NAB2=0                                                            DK050830          
      IF (NAB.EQ.2.AND.(MTY.EQ.3.OR.MTY.EQ.5.OR.MTY.EQ.7)) NAB2=1       DK050840          
      L=8                                                                                 
      CALL RCARD(IN,K,L,IC,1,NTRA)                                                        
      IF(NTRA.EQ.0) GO TO 156                                                             
      NCDS=NCDS+1                                                       DK050940          
      IF (NCD.LT.2) GO TO 32                                            DK050950          
      DO 30 J=2,NCD                                                     DK050960          
      L=J*8                                                                               
      K=L-7                                                                               
      CALL RCARD(IN,K,L,IC,J,NTRA)                                                        
      IF(NTRA.EQ.0) GO TO 903                                                             
      K1=ICN1+K-1                                                                         
      K2=K1-80                                                          DK051010          
      IF(INUM(IGETRB(IC,1,K1,48)).LE.INUM(IGETRB(IC,1,K2,48))) GO TO 901                  
...
C     ----------------------------------------------------              DK051460          
  100 CONTINUE                                                          DK051470          
      ...
      K=NDC(I)                                                          DK051510          
      IF(IGETRZ(IC,1,K,48).NE.JBLK) GO TO 102                                             
      IF (I.EQ.NMJC) GO TO 900                                          DK051530          
      IZER=IPUTR(IC,1,K,48,IZER)                                                          
  102 IMJ1=INUM(IGETRB(IC,1,K,48))+IMJ1*10                                                
      IMI1=0                                                            DK051560          
      DO 104 I=1,NMIC                                                   DK051570          
      K=NMI(I)                                                          DK051580          
      IF(IGETRZ(IC,1,K,48).NE.JBLK) GO TO 104                                             
      IF (I.EQ.NMIC) GO TO 900                                          DK051600          
      IZER=IPUTR(IC,1,K,48,IZER)                                                          
  104 IMI1=INUM(IGETRB(IC,1,K,48))+IMI1*10                                                
  ...
  110 IMJ1=0                                                            DK051700          
      ...
      K=NDC(I)                                                          DK051720          
      IF(IGETRZ(IC,1,K,48).NE.JBLK) GO TO 112                                             
      IF (I.EQ.NCC) GO TO 900                                           DK051740          
      IZER=IPUTR(IC,1,K,48,IZER)                                                          
  112 IMJ1=INUM(IGETRB(IC,1,K,48))+IMJ1*10                                                
 ...
 1054 FORMAT (1H0,38HCARDS OUT OF SEQUENCE---CHECK CARD NO.,I5,5X,15HFORDK054000          
  ...
  902 MULL=1                                                            DK054020          
      RETURN                                                            DK054030          
  903 WRITE(6,1055) J,NCDS                                                                
 1055 FORMAT(21H0EOF OR BLANK AT CARD,I3,12H OF DATA SET,I5)                              
      GO TO 902                                                                           
 ...
 1064 FORMAT (1H1,37H NO. OF MAJOR CLASSES ABSORBED BY ML=,I4,3X,21HNO. DK054300          
     ...
     3=,F5.0)                                                           DK054320          
  310 NCC=0                                                             DK054321          
      IF(NOM+NON.EQ.0) GO TO 320                                        DK054330          
 ...
 1102 FORMAT (1H )                                                      DK054430          
      ...
      SD=SQTF((ARRAY(K)-(TOT(I)*TOT(I)/ARRAY(1)))/(DF-1.))                                
      GO TO 326                                                         DK054530          
  324 SD=SQTF(ARRAY(K)/DF)                                                                
 ...
 1104 FORMAT (1H0,15X,44HOVERALL MEANS AND STANDARD DEVIATIONS OF RHM)  DK054730          
  ...
  334 IF (NAB.EQ.1.OR.NAB.EQ.2.OR.NAB.EQ.4) GO TO 336                   DK054840          
      SD=SQTF((SSCPR(J)-(TOT(K2)*TOT(K2)/ARRAY(1)))/(DF-1.))                              
      GO TO 338                                                         DK054860          
  336 SD=SQTF(SSCPR(J)/DF)                                                                
 ...
 1108 FORMAT (1H ,9HCODE CODE,9X,3HROW,22X,6HCOLUMN,23X,28HS.SQS. OR C.PDK055010          
  ...
  352 SSY2=ARRAY(K1)-(TOT(J)*TOT(J))/ARRAY(1)                           DK055150          
      RLHM=SCP/SQTF(DIV(SSY1*SSY2))                                                       
      IF (I.LT.IE.AND.J.LT.IE) GO TO 356                                DK055170          
      ARRAY(K3)=SCP                                                     DK055180          
      GO TO 356                                                         DK055190          
  354 RLHM=ARRAY(K3)/SQTF(ARRAY(K)*ARRAY(K1))                                             
 ...
 1111 FORMAT (1H0,41HRHM  LHM  RHM NAME   INDEPENDENT VARIABLE,16X,4HC.PDK055370          
  ...
  370 SSY2=SSCPR(K1)-(TOT(K4)*TOT(K4))/ARRAY(1)                         DK055560          
      RLR=SCP/SQTF(DIV(SSY1*SSY2))                                                        
      IF (J.LT.IE) GO TO 374                                            DK055580          
      RHM(K2)=SCP                                                       DK055590          
      GO TO 374                                                         DK055600          
  372 RLR=RHM(K2)/SQTF(ARRAY(K)*SSCPR(K1))                                                
 ...
 1114 FORMAT (1H0,20X,3HROW,6H   COL,4X,3HRHM,4X,3HRHM,15X,14HS.SQS. OR DK055730          
     1C.P.,4X,11HCORRELATION)                                           DK055740          
      ...
      RLHM=SCP/SQTF(SSY1*SSY2)                                                            
      GO TO 382                                                         DK055870          
  380 RLHM=SSCPR(K3)/SQTF(SSCPR(K)*SSCPR(K1))                                             
