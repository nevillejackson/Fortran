...
!     local variables
      ...
      real(8)                                 :: time,xsecs
      integer                                 :: ii,lim0,nalld,nallp,
     &                                           idelta,ibig,ierror,iq,
     &                                           i,j,k,mxspaa,m
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      allocate (iwork(0:neqns),nnvec(neqns),stat=ii)
      if(ii>0)stop 'dvord3 : alloc NNVEC'

      write(*,*)'give value for "MAXSPA"  '
      m=(neqns/10+1)*750
      call optdef(mxspaa,1,999999999,m)

      allocate(ixsub(mxspaa),stat=ii)
      if(ii>0)stop 'dxord3 : allocate ixsub'

      allocate (kfirst(neqns+1),kvcol(mxspaa+mxspaa),stat=ii)
...
1     continue
      ...
      write(*,*)' "dvord3" : new no. of rows                  =',nsrow
...
!     allocate arrays depending on nsrow
      ...
      if(ieqmod.eq.1)then ! need phi in gg_inverse
         k=maxval(kftfix)
         m=max0(mfitmx,k)
         allocate(phi(mage,m,0:5,nq),stat=ii)
         if(ii>0)stop 'alloc : phimatrix'
         call all_legendre (m)
         phi=0.d0
         do iq=1,nq
         if(kfit(4,iq)>0)call phimat(nage(irrmet(4,iq)),kfit(4,iq),4,iq)
         end do
      end if
...
!     look for new order set up in previous, incomplete run
      ...
      if(ksrow.eq.nsrow .and. keqns.eq.neqns )then
         write(*,'(a,2i6)')'found file "49" - use info ? ',ksrow,keqns
         call yndef(ii,1)
         if(ii.eq.1)then
            read(iun49)iflag
            read(iun49)invprm
...
C     SET UP ADJACENCY STRUCTURE OF COEFFICIENT MATRIX 
 99   continue
      CALL DxADJ3(NALLD,NALLP,0,mxspaa)
...
C     DETERMINE MINIMUM DEGREE ORDERING 
     ...
     *                                                          mkvcol)
      write(*,*)' end of genmmd',nsrow,'  nofsub=',nofsub
...
C     REBUILD ADJACENCY STRUCTURE ADDING EXTRA ROW/COL
 199  NSROW=NSROW+1
      CALL DxADJ3(NALLD,NALLP,1,mxspaa)
      ivperm(nsrow)=nsrow
      invprm(nsrow)=nsrow
...
C     CARRY OUT SYMBOLIC FACTORISATION & SET UP COMPRESSED  STORAGE SCHEME     
     ...
     *            ixvec2,ixsub,maxsub,iwork,ieqnew,nnvec,ierror)
      ...
      write(*,'(a,2i10)')'end of smbfct : maxsub',maxsub
      write(*,'(a,2i10)')'                maxlnz',maxlnz

      if(ierror.gt.0)then
         write(*,*)'routine "SMBFCT" : too many subscripts !'
         write(*,*)'max. allowed   =',mxspaa
         write(*,*)'no. required   =',maxsub
         stop 'rerun program with bigger "maxspa" !'
      end if
...
C     --------------------------------
      ...
      REWIND(IUN22)
      REWIND(IUN52)
      CALL DFTIME(time,xsecs,isecs,13,'DVORD3')
      allocate (iposfx(mfix,nq),stat=ii)
      if(ii>0)stop 'all iposfx'
      ...
      CALL DFMME3(mfix,nq,IPOSFX)
      deallocate(iposfx,stat=ii)
